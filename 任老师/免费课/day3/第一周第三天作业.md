...
-  一、输出下面的运行结果
    ...
    + 6、(这个可以做不出来，可以自己查查资料)
        ```
        let i=2;  3  4  5
        console.log(i++ == ++i) ; //false 2 == 4  
        console.log(++i == i++);  // true 5 ==5
        console.log(i); //6
...
- 二、输出下面的结果
    ...
    3、 Boolean(Number("")) + !isNaN(Number(null)) + Boolean("parseInt([])") 
...
- 四、输出以下结果(遇见报错不用管，继续执行)
    + 1、
        ...
        //'weiWei'  'liaoLiao'   {name ='weiWei',ifantName='liaoLiao'}
        ```
    + 2、(画堆栈内存图)
